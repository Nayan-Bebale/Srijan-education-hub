...
// Define WebSocket protocol based on the current URL protocol
let wsStart = 'ws://'; // For non-secure WebSocket connections (ws://)
if (window.location.protocol === 'https:') {
    wsStart = 'wss://'; // For secure WebSocket connections (wss://)
}

// Construct the WebSocket endpoint
let endpoint = wsStart + window.location.host + window.location.pathname;

// Create a new WebSocket connection
let socket = new WebSocket(endpoint);

// WebSocket event listener for when the connection is open
socket.onopen = function(event) {
    ...
    send_message_form.on('submit', function(event) {
        ...
        // Get message, recipient, and thread information
